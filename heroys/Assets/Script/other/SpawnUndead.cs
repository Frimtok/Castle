...
public class SpawnUndead : MonoBehaviour
{
    ...
    private float AddTime(float time)
    {
      return time *= 2;
    }
    private void SapwnUndead(SpawnObject spawnObject)
    {
            ...
            {
                Instantiate(spawnObject.Undead.gameObject,new Vector2(transform.position.x + i, transform.position.y + 0.2f),Quaternion.identity);
            }
    }
